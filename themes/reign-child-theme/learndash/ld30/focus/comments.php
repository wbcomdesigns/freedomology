...
<div class="ld-focus-comments">
	...
	$learndash_comment_count = wbcom_get_filtered_comment_count( get_the_id() );

	if ( ( ! empty( $learndash_comment_count ) ) && ( ! isset( $_GET['replytocom'] ) ) ) {
		?>
			...
			<div class="ld-focus-comments__header">
				...
				printf(
					esc_html(
						...
						_nx( '%s Comment', '%s Comments', $learndash_comment_count, 'comments', 'learndash' )
					),
					esc_html( number_format_i18n( $learndash_comment_count ) )
				);
				?>
			</div>
