						...
						if ( $remove_user_button ) {
							...
							<div id="group-management-enrolled-users-table-remove-users"
								...
								<button class="ulgm-modal-link uo-btn uo-left"
										...
										rel="modal:open"
										tabindex="0"><?php echo GroupManagementInterface::$ulgm_management_shortcode['text']['remove_users']; ?></button>
							</div>
					...
					if (
						...
						<?php if ( is_array( $users_menu_items ) && ! empty( $users_menu_items ) ) { ?>
							<div class="uo-groups-list-of-btns uo-inline-block uo-left" id="uo_add_users_button"
								data-id="add-users">
								<button class="uo-btn uo-groups-list-of-btns-main" data-direction="<?php echo is_rtl() ? 'left' : 'right'; ?>"
									data-id="add-users" tabindex="0">
								...
								<div class="uo-groups">
									<div class="group-management-form">
										<form id="group-management-add-user-frm" action="return false;">
											...
											<div class="uo-row" id="uo_add_user_first_name">
											...
											<div class="uo-row" id="uo_add_user_last_name">
												<label for="last-name">
													<div class="uo-row__title">
											...
											<div class="uo-row" id="uo_add_user_email">
												<label for="email">
													<div class="uo-row__title">
								...
								<div class="group-management-form">
									...
									<form id="group-management-edit-user-frm" action="return false;">
										...
										<div class="uo-row <?php echo ! $email_field_editing_allowed ? ' uo-row-disabled' : ''; ?>">
											...
											<?php if ( $email_field_editing_allowed ) : ?>
											<input class="uo-input" required="required" type="text" name="email"
													id="edit-email" value="">
											<?php else : ?>
											<input class="uo-input" required="required" type="text" name="email" readonly 
													id="edit-email" value="">
											<?php endif; ?>
										</div>
				...
				<div class="group-management-buttons__right">
					...
					if (
						...
						<div class="uo-groups-list-of-btns uo-inline-block uo-right"
							...
							<button class="uo-btn uo-right uo-groups-list-of-btns-main" id="uo-groups-action-users"
								data-direction="<?php echo is_rtl() ? 'right' : 'left'; ?>" data-id="general-actions" tabindex="0">
					...
					<div>
						<label class="uo-radio">
							<input class="uo-radio-input bb-custom-check" type="radio" name="action"
									id="add-invite" value="add-invite" checked="checked">
							<span class="uo-radio-checkmark"></span>
						</label>
		...
		>
			<div class="uo-groups">
				<div class="group-management-form">
					...
					<div class="uo-row uo-row--group-management-email-users-from-name">
						<label for="group_email_fromname">
							<strong><?php esc_html_e( 'From name:', 'uncanny-learndash-groups' ); ?></strong>
							<?php echo esc_attr( SharedFunctions::ulgm_group_management_email_users_from_name( GroupManagementInterface::$ulgm_current_managed_group_id, get_current_user_id() ) ); ?>
						</label>
					</div>

					<div class="uo-row uo-row--group-management-email-users-from-email">
						<label for="group_email_fromemail">
							<strong><?php esc_html_e( 'From email:', 'uncanny-learndash-groups' ); ?></strong>
							<?php echo esc_attr( SharedFunctions::ulgm_group_management_email_users_from_email( GroupManagementInterface::$ulgm_current_managed_group_id, get_current_user_id() ) ); ?>
						</label>
					</div>

					<div class="uo-row uo-row--group-management-email-users-reply-to">
						...
						<select id="ulgm-management-reply-to-email" name="reply_to_email">
							...
							foreach ( $group_leaders as $group_leader ) {
								...
								<?php printf( '%s %s [%s]', $g_user->first_name, $g_user->last_name, $g_user->user_email ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
								</option>
								<?php
							}
							?>
					...
					<div class="uo-row uo-row--group-management-email-users-courses">
						<label for="group_email_replytoemail">
							<div class="uo-row__title">
					...
					<div class="uo-row uo-row--group-management-email-users-course-statuses">
						<label for="group_email_replytoemail">
							<div class="uo-row__title">
						...
						<select
							...
							multiple="multiple"
							id="ulgm-management-send-email-course-statuses"
						>
							<?php
					...
					<div class="uo-row uo-row--group-management-email-users-email-subject">
					...
					<div class="uo-row uo-row--group-management-email-users-email-message">
						<label for="group_email_text">
							<div class="uo-row__title">
					...
					<?php
					// Show the "Include students in child groups" option if group is a parent and has child groups..
					if ( SharedFunctions::has_children_in_group( GroupManagementInterface::$ulgm_current_managed_group_id ) ) {
						?>
						<div class="uo-row uo-row--group-management-email-users-email-include-children">
							<label class="uo-checkbox">
								<input class="uo-checkbox-input bb-custom-check" type="checkbox"
										name="group_email_include_children" id="group_email_include_children"
										value="group_email_include_children">
								<span class="uo-checkbox-checkmark"></span>
							</label>

							<label for="group_email_include_children">
								<?php esc_html_e( 'Include students in child groups', 'uncanny-learndash-groups' ); ?>
							</label>
						</div>
					<?php } ?>

					<div class="uo-row-footer uo-row--group-management-email-users-submit">
						<div class="ulgm-notice ulgm-notice--info uo-row--group-management-email-users-user-count">
							<?php printf( esc_html__( 'This email will be sent to %1$s user(s).', 'uncanny-learndash-groups' ), '<span id="ulgm-management-email-users-count"></span>' ); ?>
						</div>
